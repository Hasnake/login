...
def login():
    ...
    if request.method == 'POST':
        # How do we get the password of the current user?
        # How does having the current user's email help?
            # How can we use the user email to get the user password?

        current_email = session['email']
        current_user = db.session.query(User).filter_by(email=current_email).first()

        if current_user != None and request.form['password'] == current_user.password and request.form['email'] == current_user.email:
            session['login'] = True
        else:
...
# The user sends in their username and password (plus their password confirmation)
# They're doing it BECAUSE they want to register.

# When they register, they can login, logout, and save data. (Unregistered users can't do that.)

# If we (the developers) want a user to be able to login,
# then we have to add the user to the database AND make a
# cookie to keep track of the user.

# If we want a user to be able to logout, then we have to
#     1. Get rid of the cookie.
#     2. Create logout button
#         a. Make it so something submits when the button is clicked (make a form for the button)
#         b. <form action="/register"> </form>

# If we want a user to be able to login, then we have to
#    _____
